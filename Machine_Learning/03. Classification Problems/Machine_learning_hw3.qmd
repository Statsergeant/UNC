...
> set.seed(1234)
> tran 
<- sample(dim(College)\[1\], floor(dim(College)\[1\]\*0.7))
\end{verbatim}
```
...
Build 4 classifiers such as LR (Logistic Regression), LDA(Linear Discriminant Analysis), QDA(Quadratic Discriminant Analysis) and NB(Naive Bayes) based on the training set. Compute the misclassification
rate of the test set. Use the threshold of 0.5 for LR.
...
Separate 543 training observations into 10 folds such that
...
\end{verbatim}
```
Perform 10-fold cross validation (CV) based on `gr` to find the optimal threshold of the prediction probability each classifier. For example, we can classify the jth observation as “Yes” if 
$$
P(y_j = \text{'Yes'} \mid x_j) > \alpha
$$ 
where $\alpha$ ∈ [0, 1] is a threshold. Let $\alpha$ starts from 0 to 1 increased by 0.01. The optimal threshold minimizes the misclassification rate of the 10-fold CV. If the minimum of the misclassification rate is achieved at multiple $\alpha$ values, the optimal $\alpha$ should be computed as the mean of the $\alpha$ values that minimize the misclassification rate. Find the optimal threshold for each classifier and then apply the optimal threshold to compute the misclassification rate of the test set. Provide the misclassification rate of the test set along with the optimal threshold for each classifier.
...
Let us define F1 score ($F_1$) by

$$
F_1 = \frac{2 \times \text{TP}}{2 \times \text{TP} + \text{FP} + \text{FN}}
$$
Repeat Q3 except that the optimal threshold maximizes $F_1$ of the 10-fold CV. If the maximum of $F_1$ is achieved at multiple $\alpha$ values, the optimal $\alpha$ should be computed as the mean of the $\alpha$ values that maximize $F_1$ Find the optimal threshold for each classifier and then apply the optimal threshold to compute $F_1$ of the test set. Provide $F_1$ of the test set along with the optimal threshold for each classifier.
...
<p class='Tabletitle'>The optimal $\hat{\alpha}$</p>

$$
\begin{array}{c|c|c|c}
 & \text{LR} & \text{LDA} & \text{QDA} & \text{NB} \\
\hline
\text{Q3} & \dots & \dots & \dots & \dots  \\
\text{Q4} & \dots & \dots & \dots & \dots  \\
\text{Q5} & \dots & \dots & \dots & \dots  \\
\end{array}
$$

<p class='Tabletitle'>Prediction Performance</p>

$$
\begin{array}{c|c|c|c|c}
 & \text{LR} & \text{LDA} & \text{QDA} & \text{NB} & \textbf{Winner}\\
\hline
\text{Q1 (Mis)} & \dots & \dots & \dots & \dots & \dots \\
\text{Q2 (AUC)} & \dots & \dots & \dots & \dots & \dots \\
\text{Q3 (Mis)} & \dots & \dots & \dots & \dots & \dots \\
\text{Q4 (MCC)} & \dots & \dots & \dots & \dots & \dots \\
\text{Q5 ($F_1$)} & \dots & \dots & \dots & \dots & \dots \\
\end{array}
$$
...
q6.alpha %>% kable() %>% kable_styling(full_width = F)
...
q6.perf %>% kable() %>% kable_styling(full_width = F)
